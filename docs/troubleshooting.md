...
| Console vanishes at "Loading Whisper AI model"; Event Viewer shows `MSVCP140.dll` / exception `0xc0000005` | Visual C++ runtime older than 14.40 (common on older Windows 10 installs) — the Whisper engine hard-crashes against it | Install the latest [vc_redist.x64.exe](https://aka.ms/vs/17/release/vc_redist.x64.exe) and relaunch. The app warns about this at startup, and `whisper-local --doctor` checks it |
