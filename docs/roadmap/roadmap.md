...
### Packaging & Updates
- As a *Windows user* I want a **proper installer (`whisper-local-setup.exe`)** that installs to `Program Files` (or `%LOCALAPPDATA%\Programs` for no-admin), adds a Start Menu shortcut, and registers an uninstaller in Add/Remove Programs so I get a familiar install/uninstall experience instead of a portable exe running out of my Downloads folder — field feedback 2026-07-06: first-time user expected the downloaded exe to install into `C:\Program Files`. Likely Inno Setup wrapping the pyapp exe; keep the portable exe as a second release asset; uninstaller should offer to remove `%LOCALAPPDATA%\pyapp\data\whisper-local` and `%APPDATA%\whisperkey`; should install/verify the VC++ redistributable (≥14.40 — see LEARNINGS 2026-07-06 MSVCP140 crash); unblocks winget `installer` type
